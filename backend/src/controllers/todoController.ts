...
export function getCategories(req: Request, res: Response) {
  try {
    const categories = db.prepare('SELECT name FROM categories').all() as Category[];
    res.json(categories);
  } catch (error) {
...
export function createTodo(req: Request, res: Response) {
  try {
    ...
    const newTodo = db.prepare('SELECT * FROM todos WHERE id = ?').get(info.lastInsertRowid) as Todo;
...
export function getTodos(req: Request, res: Response) {
  try {
    ...
    let todos: Todo[];
    if (category) {
      todos = db.prepare('SELECT * FROM todos WHERE category = ? ORDER BY id DESC').all(category) as Todo[];
    } else {
      todos = db.prepare('SELECT * FROM todos ORDER BY id DESC').all() as Todo[];
    }
